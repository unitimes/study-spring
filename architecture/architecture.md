...
특정 계층에 종속되지 않으면서 애플리케이션 전반에서 사용

오브젝트를 **정보**를 담는 목적으로만 사용할 지 관련 **비즈니스 로직**까지 담을 지에 따라 다시 구분

###데이터와 오브젝트
데이터 중심의 아키텍처는 DAO가 만드는 SQL의 결과가 데이터가 되며, 모든 계층의 코드가 의존하게 됨    
...
###정보 전송 아키텍처
프레젠테이션 계층의 MVC 아키텍처에서도 모델은 도메인 오브젝트를 그대로 사용
